...
usage()
{
    echo "usage: ./create_server_cert.sh <FILENAME> <KEY_LENGTH> <DAYS>"
    exit 1
}
...
# Create certificate
openssl ca -config intermediate.cnf -extensions server_cert -days $DAYS -notext -md sha256 -in intermediate/csr/$FILENAME.csr -out intermediate/certs/$FILENAME.crt
